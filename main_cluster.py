...
if server == "rpbs":
    ...
    MainPath = ""
    ifb_project = os.getcwd().split('projects')[1].split('/')[1]
...
def get_free_disk():
    quotas = str(subprocess.check_output(["bash scripts/getquota2.sh "+ifb_project], shell=True)).strip().split()
...
os.system("(echo && echo \"==========================================\" && echo && echo \"CLUSTER\" && echo) | cat - cluster.yml >>" + LogPath+time_string+"_configuration.txt")
...
if qc=='yes':
    ...
    if trim=='yes':
        ...
        start_time = time.time()
        os.system("snakemake -k --cluster-config cluster.yml --drmaa \" --mem={cluster.mem} -J {cluster.name} -c {cluster.cpus}"+option+"\" --use-conda --conda-prefix "+MainPath+".snakemake/conda/ --jobs=30 --latency-wait 40 -s "+MainPath+"workflow/trim.rules 2> " + LogPath+time_string+"_trim.txt")
        end_time = time.time()
    ...
    if mapping =='yes' and reference == "transcriptome":
        ...
        start_time = time.time()
        os.system("snakemake -k --resources load=100 --cluster-config cluster.yml --drmaa \" --mem={cluster.mem} -J {cluster.name} -c {cluster.cpus}"+option+"\" --use-conda --conda-prefix "+MainPath+".snakemake/conda/ --jobs=30 --latency-wait 40 -s "+MainPath+"workflow/quantify_trans.rules 2> " + LogPath+time_string+"_quantify_trans.txt")
        end_time = time.time()
    ...
    if mapping =='yes' and reference == "genome":
        ...
        #os.system("snakemake -k --resources load=100 --cluster-config cluster.yml --drmaa \" --mem={cluster.mem} -J {cluster.name} -c {cluster.cpus}"+option+"\" --use-conda --conda-prefix "+MainPath+".snakemake/conda/ --jobs=30 --latency-wait 40 -s "+MainPath+"workflow/align_count_genome.rules 2> " + LogPath+time_string+"_align_count_genome.txt")
        os.system("snakemake --resources load=100 --cluster-config cluster.yml --drmaa \" --mem={cluster.mem} -J {cluster.name} -c {cluster.cpus}"+option+"\" --use-conda --conda-prefix "+MainPath+".snakemake/conda/ --jobs=30 --latency-wait 40 -s "+MainPath+"workflow/align_count_genome.rules 2> " + LogPath+time_string+"_align_count_genome.txt")
        end_time = time.time()
    ...
    if dea=='yes':
        ...
        if reference == "transcriptome":
            start_time = time.time()
            os.system("snakemake -k --cluster-config cluster.yml --drmaa \" --mem={cluster.mem} -J {cluster.name} -c {cluster.cpus}"+option+"\" --use-conda --conda-prefix "+MainPath+".snakemake/conda/ --jobs=30 --latency-wait 40 -s "+MainPath+"workflow/dea_trans.rules 2> " + LogPath+time_string+"_dea_trans.txt")
            end_time = time.time()
            file_main_time.write("Time of running DEA transcriptome based: " + spend_time(start_time, end_time) + "\n")
        elif reference == "genome":
            start_time = time.time()
            os.system("snakemake -k --cluster-config cluster.yml --drmaa \" --mem={cluster.mem} -J {cluster.name} -c {cluster.cpus}"+option+"\" --use-conda --conda-prefix "+MainPath+".snakemake/conda/ --jobs=30 --latency-wait 40 -s "+MainPath+"workflow/dea_genome.rules 2> " + LogPath+time_string+"_dea_genome.txt")
            end_time = time.time()
